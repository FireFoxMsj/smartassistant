...
func NewAttr(attr, name string) Attribute {
...
func NewDeviceControl(deviceID int, attrs ...Attribute) []Permission {
	target := DeviceTarget(deviceID)
	res := make([]Permission, 0)
	for _, attr := range attrs {
		name := fmt.Sprintf("控制%s", attr.name)
		p := Permission{name, "control", target, attr.attr}
		res = append(res, p)
	}
	return res
}
