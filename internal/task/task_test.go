...
func TestMain(m *testing.M) {
	...
	sFunc := func(task *Task) error {
		log.Println("running startup task after 59 sec")
		return nil
	}
	ta := NewTask(sFunc, 59*time.Second)
	GetManager().PushTask(ta, "task to keep queue running")

	go GetManager().queue.Start(ctx)
	code := m.Run()
	<-ctx.Done()
	config.TestTeardown()
	os.Exit(code)
...
func TestNewTask(t *testing.T) {
	...
	GetManager().PushTask(ta, "test new task")
...
func TestNewTaskSameDelay(t *testing.T) {
	...
	GetManager().PushTask(ta, "test new task a")
	GetManager().PushTask(tb, "test new task b")
	time.Sleep(3 * time.Second)
...
func TestNewTaskAt(t *testing.T) {
	...
	GetManager().PushTask(ta, "test new task at")
	time.Sleep(3 * time.Second)
...
func TestTaskWithParent(t *testing.T) {
	...
	tFuncP := func(task *Task) error {
		...
		GetManager().PushTask(tc, "test child task")
		return nil
	}
	tp := NewTask(tFuncP, 1*time.Second)
	GetManager().PushTask(tp, "test parent task")
...
func TestTaskWithWrapper(t *testing.T) {
	...
	GetManager().PushTask(ta, "test task with wrapper")
	time.Sleep(3 * time.Second)
