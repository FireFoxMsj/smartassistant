...
func InitConfig(fn string) *Options {
	fd, err := os.Open(fn)
	if err != nil {
		logrus.Panic(fmt.Sprintf("open conf file:%s error:%v", fn, err.Error()))
	}
	defer fd.Close()

	content, err := ioutil.ReadAll(fd)
	if err != nil {
		logrus.Panic(fmt.Sprintf("read conf file:%s error:%v", fn, err.Error()))
	}

	if strings.HasSuffix(fn, ".json") {
		if err = jsoniter.Unmarshal(content, &options); err != nil {
			logrus.Panic(fmt.Sprintf("unmarshal conf file:%s error:%v", fn, err.Error()))
		}
