...
func TestAddManagerRole(t *testing.T) {
	...
	role, err := AddManagerRoleWithDB(GetDB(), managerRoleName)
	...
	_, err = AddManagerRoleWithDB(GetDB(), lengthGreaterThan20RoleName)
...
func TestIsRoleNameExist(t *testing.T) {
	...
	for _, t := range ts {
		ast.Equal(t.expectedRes, IsRoleNameExist(t.name, t.roleID))
	}
}
