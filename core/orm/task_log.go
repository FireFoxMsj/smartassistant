...
func UpdateTaskLog(taskID string, taskErr error) error {
	...
	if taskErr != nil {
		...
		if v, ok := taskErr.(errors.Error); ok { // 判断错误类型
			switch v.Code {
			case errors.SceneNotExist:
				update.Result = TaskSceneAlreadyDeleted
			case errors.DeviceNotExist:
				update.Result = TaskDeviceAlreadyDeleted
			case errors.DeviceOffline:
				update.Result = TaskDeviceDisConnect
			}
		}
