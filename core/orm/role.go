...
import (
	...
	errors2 "gitlab.yctc.tech/root/smartassistent.git/utils/errors"
...
func (r *Role) BeforeCreate(tx *gorm.DB) (err error) {
	if utf8.RuneCountInString(r.Name) > 20 {
		err = errors2.Wrapf(err, errors2.NameSizeLimit, "角色名称", 20)
	}
	return
}
...
func (r *Role) BeforeUpdate(tx *gorm.DB) (err error) {
	...
	if r.IsManager {
		return errors2.New(errors2.Deny)
	}
	return nil
}
...
func GetRoleByID(id int) (role Role, err error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			err = errors2.Wrap(err, errors2.RoleNotExist)
		} else {
...
func (r *Role) AddPermissions(ps ...permission.Permission) {
	r.AddPermissionsWithDB(GetDB(), ps...)
}
func (r *Role) AddPermissionsWithDB(db *gorm.DB, ps ...permission.Permission) {
...
func (r *Role) addPermission(db *gorm.DB, p permission.Permission) error {
...
func (r *Role) DelPermission(p permission.Permission) error {
...
func InitRole(db *gorm.DB) (err error) {
	...
	manager.AddPermissionsWithDB(db, permission.ManagerPermission...)

	member, err = AddRoleWithDB(db, "成员")
	if err != nil {
		return err
	}
	member.AddPermissionsWithDB(db, permission.MemberPermission...)
